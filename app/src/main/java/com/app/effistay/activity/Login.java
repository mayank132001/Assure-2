...
public class Login extends AppCompatActivity {
    ...
    public void login(final String sendEmail, final String sendPassword, String type) {
        ...
        requestBody.put("customer_email", sendEmail);
        requestBody.put("password", sendPassword);
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<LoginRP> call = apiService.getLogin(requestBody);
...
//                    assert loginRP != null;
                    Log.v("", "=== onResponse " + loginRP.getResponseCode());
                    if (loginRP.getResponseCode() == 1) {
                        Log.v("", "=== loginRP " + loginRP.getResponseData().getName());
//                        if (loginRP.getSuccess().equals("1")) {
                        ...
                        method.editor.putString(method.profileId, loginRP.getResponseData().getUserId());
                        method.editor.putString(method.userName, loginRP.getResponseData().getName());
                        method.editor.putString(method.userEmail, loginRP.getResponseData().getEmail());
                        method.editor.putString(method.userPhone, loginRP.getResponseData().getPhone());
                        ...
                        method.alertBox(loginRP.getResponseText());
                    }
...
//                    }
                    ...
                    Log.v("fail", "=== onfalid " + e.getMessage());
    ...
    public void registerSocialNetwork(String id, String sendName, String sendEmail, final String type) {
        ...
        requestBody.put("social_type", type);
        requestBody.put("phone", "");
        requestBody.put("address", "");
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<RegisterRP> call = apiService.getSocialRegister(requestBody);
        call.enqueue(new Callback<RegisterRP>() {
            ...
            public void onResponse(@NotNull Call<RegisterRP> call, @NotNull Response<RegisterRP> response) {
                ...
                try {
                    ...
                    Log.v("", "==== Social reg " + registerRP.getResponseCode()+" "+registerRP.getResponseText());
//                    if (registerRP.getResponseCode() == 1) {
//                        Log.v("", "==== email res " + registerRP.getResponseData().getUserId());
////                        if (registerRP.getSuccess().equals("1")) {
//
//                        method.editor.putBoolean(method.prefLogin, true);
//                        method.editor.putString(method.profileId, registerRP.getResponseData().getUserId());
//                        method.editor.putString(method.loginType, type);
//                        method.editor.commit();
//
//                        Toast.makeText(Login.this, registerRP.getResponseText(), Toast.LENGTH_SHORT).show();
//
//                        if (Method.loginBack) {
//                            Method.loginBack = false;
//                            onBackPressed();
...
//                    }
    ...
    public void socialLogin(final String sendEmail, String type) {

        Map<String, String> requestBody = new HashMap<>();
        requestBody.put("email", sendEmail);
        requestBody.put("social_type", type);
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<LoginRP> call = apiService.getSocialLogin(requestBody);
        call.enqueue(new Callback<LoginRP>() {
            @Override
            public void onResponse(@NotNull Call<LoginRP> call, @NotNull Response<LoginRP> response) {
                ...
                try {
                    ...
                    assert loginRP != null;
                    Log.v("", "==== Social reg " + loginRP.getResponseCode()+" "+loginRP.getResponseText());
                    if (loginRP.getResponseCode() == 1) {
                        Log.v("", "=== loginRP " + loginRP.getResponseData().getName());
//                        if (loginRP.getSuccess().equals("1")) {
